...
void main() {
  ...
  group("What's New in v2.4.0 (Build 75) Sheet Tests", () {
    testWidgets('1. WhatsNewSheet renders version badge, all 5 feature cards, and CTA button', (tester) async {
      ...
      expect(find.text('PHIÊN BẢN MỚI v2.4.0 (BUILD 75)'), findsOneWidget);
      expect(find.text('Trải Nghiệm Hội Thoại & Tính Năng Mới'), findsOneWidget);

      // Check feature cards
      expect(find.text('Bộ ba Thao tác Tin nhắn'), findsOneWidget);
      expect(find.text('Chạm Trích dẫn Nhảy Tin nhắn gốc'), findsOneWidget);
      expect(find.text('Đồng bộ Ảnh & Xem trước 0.00s'), findsOneWidget);
      expect(find.text('Thẻ Tệp Zalo & Shared Media Hub'), findsOneWidget);
      expect(find.text('Tăng tốc & Báo Đang soạn tin'), findsOneWidget);
    ...
    testWidgets('2. WhatsNewSheet dismisses when tapping CTA button', (tester) async {
      ...
      expect(find.text('PHIÊN BẢN MỚI v2.4.0 (BUILD 75)'), findsNothing);
    });
  });
}
