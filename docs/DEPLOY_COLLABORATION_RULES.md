...
> [!IMPORTANT]
> **Tài liệu quy chuẩn bắt buộc**: Áp dụng cho mọi phiên làm việc phối hợp giữa **Sếp**, **Sếp Châu**, **Đội vận hành (Backend Deployment Lead)** và **AI Assistant**.
...
| :--- | :--- | :--- | :--- |
| **Sếp** | **Frontend Deployment Lead** | Nguồn release mobile source | - Trực tiếp kiểm tra giao diện và đồng bộ mã nguồn vào release mobile.<br>- Kích hoạt **GitHub Actions CI/CD** đóng gói bản build TestFlight / App Store cho iPhone 13. |
...
>    - **Hậu quả nghiêm trọng**: Nếu Đội vận hành tự ý thay đổi logic backend hoặc contract, toàn bộ **hệ thống test suite (207 tests)** trên máy Sếp sẽ bị **FAIL (LỖI ĐỎ)** ngay lập tức. Khi test đỏ, luồng **GitHub Actions CI/CD của Frontend sẽ tự động BỊ CHẶN ĐỨNG (Build Failed)** và **HOÀN TOÀN KHÔNG THỂ DEPLOY hay phát hành bản dựng mới được**.
...
>    - Tuyệt đối **KHÔNG** sửa thẳng vào mã nguồn trước khi được Sếp và team đồng thuận.
...
│    - Sếp merge code vào nguồn `release/ios-appstore`.              │
...
│    - iPhone 13 của Sếp nhận bản cập nhật TestFlight mới.           │
